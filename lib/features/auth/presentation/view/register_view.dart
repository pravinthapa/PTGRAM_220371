...
// import 'package:prabin/features/batch/domain/entity/batch_entity.dart';
...
class _RegisterViewState extends State<RegisterView> {
  final _gap = const SizedBox(height: 8);
  final _key = GlobalKey<FormState>();
  final _fnameController = TextEditingController(text: 'Prabin');
  ...
  final _passwordController = TextEditingController(text: 'Prabin123456');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Register Student'),
        centerTitle: true,
        backgroundColor: Colors.deepOrange, // Instagram-like color
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  BlocBuilder<BatchBloc, BatchState>(builder: (context, state) {
                    return DropdownButtonFormField<BatchEntity>(
                      items: state.batches
                          .map((e) => DropdownMenuItem<BatchEntity>(value: e, child: Text(e.batchName)))
                          .toList(),
                      onChanged: (value) {
                        _dropDownValue = value;
                      },
                      ...
                      decoration: InputDecoration(
                        labelText: 'Select Batch',
                        labelStyle: TextStyle(color: Colors.deepOrange), // Instagram-like color
                      ),
                      validator: ((value) {
                        if (value == null) {
                          return 'Please select batch';
                  ...
                  _gap,
                  BlocBuilder<CourseBloc, CourseState>(builder: (context, courseState) {
                    if (courseState.isLoading) {
                      return const CircularProgressIndicator();
                    } else {
                      return MultiSelectDialogField(
                        ...
                        items: courseState.courses
                            .map(
                              (course) => MultiSelectItem(course, course.courseName),
                            )
                            .toList(),
                        listType: MultiSelectListType.CHIP,
                        buttonText: const Text(
                          'Select course',
                          style: TextStyle(color: Colors.deepOrange), // Instagram-like color
                        ),
                        buttonIcon: const Icon(Icons.search),
                        onConfirm: (values) {
                          _lstCourseSelected.clear();
                        ...
                        decoration: BoxDecoration(
                          border: Border.all(
                            color: Colors.deepOrange, // Instagram-like color
                          ),
                          borderRadius: BorderRadius.circular(5),
                        ),
                        validator: ((value) {
  ...
  // Custom text field builder to reduce repetition
